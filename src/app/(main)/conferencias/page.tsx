...
import { placeholderConferences, type Conference, simulateFetch } from '@/lib/placeholder-data'
import { ConferenceCard, ConferenceCardSkeleton } from '@/components/conferencias/ConferenceCard'
import { Input } from '@/components/ui/input'
import { Search, Facebook, Instagram, Linkedin, Youtube as YoutubeIcon, Video } from 'lucide-react'
...
export default function ConferenciasPage() {
  ...
  useEffect(() => {
    const fetchPageData = async () => {
      setLoading(true);
      const sortedConferences = placeholderConferences.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
      const fetchedConferences = await simulateFetch(sortedConferences, 700);
  ...
  const filteredConferences = useMemo(() => {
    ...
    return conferences.filter(conf =>
      conf.title.toLowerCase().includes(lowerSearchTerm) ||
      conf.description.toLowerCase().includes(lowerSearchTerm)
    );
    ...
    <div className="space-y-12">
      <header className="text-center">
        <h1 className="text-4xl font-bold tracking-tight text-primary sm:text-5xl flex items-center justify-center gap-3">
          <Video className="h-10 w-10" /> Mis Conferencias
        </h1>
        <p className="mt-4 text-lg text-foreground/80 max-w-2xl mx-auto">
          Explora una colección de mis charlas y presentaciones sobre literatura, escritura y educación.
        </p>
      </header>
      ...
      <div className="relative max-w-lg mx-auto">
        <Input
          type="text"
          placeholder="Buscar conferencias por título o descripción..."
          value={searchTerm}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1, 2, 3].map((i) => <ConferenceCardSkeleton key={i} />)}
        </div>
        ...
        <p className="text-center text-muted-foreground py-10">
          {searchTerm ? `No se encontraron conferencias para "${searchTerm}".` : "Actualmente no hay conferencias disponibles. ¡Vuelve pronto!"}
        </p>
      )}
    </div>
  )
