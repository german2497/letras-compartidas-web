...
import type { GameDocumentComment } from '@/lib/placeholder-data'
...
interface GameDocumentCommentSectionProps {
  documentId: string;
  initialComments: GameDocumentComment[];
  onCommentSubmit: (content: string) => Promise<GameDocumentComment | null>;
}

export function GameDocumentCommentSection({ documentId, initialComments, onCommentSubmit }: GameDocumentCommentSectionProps) {
  const { user } = useAuth()
  const { toast } = useToast()
  const [comments, setComments] = useState<GameDocumentComment[]>(initialComments)
  ...
  const handleSubmitComment = async (e: FormEvent) => {
    ...
    if (submittedComment) {
      ...
      setNewComment('')
      toast({ title: "Comentario añadido al documento" })
    } else {
  ...
  return (
    <div className="mt-8 space-y-6">
      <h3 className="text-2xl font-semibold text-primary border-b pb-2">Comentarios sobre el Documento ({comments.length})</h3>
      ...
      <div className="space-y-6">
        {comments.length > 0 ? comments.map((comment) => (
          ...
          <p className="text-muted-foreground text-center py-4">No hay comentarios aún para este documento. ¡Sé el primero en comentar!</p>
        )}
      </div>
    </div>
  )
